...
then stitches the videos together and uploads to Google Drive.
"""
...
UPLOAD_TO_GOOGLE_DRIVE = True  # Enable/disable Google Drive upload
QUEUE_PROCESSING_COMMAND = True  # Enable/disable command queue
PROCESSING_INTERVAL = 40  # Frame interval for processing (used in command queue)
...
class Scanner:
    ...
    def run(self):
        ...
        try:
            ...
            # Upload to Google Drive and queue processing command
            if UPLOAD_TO_GOOGLE_DRIVE:
                if not self._upload_to_google_drive():
                    print("\n[WARNING] Google Drive upload failed, but scan completed locally.")
                    print(f"Local file: {self.recorder.final_file}")
    ...
    def _upload_to_google_drive(self):
        """
        Upload the final MP4 to Google Drive and queue processing command.
        ...
        try:
            ...
            print("[DRIVE] Initializing Google Drive connection...")
            storage = GoogleDrivePointCloudStorage()

            # Generate a unique name for this scan
            scan_name = f"scan_{self.recorder.timestamp}"

            # Upload the MP4 file
            print(f"[DRIVE] Uploading {scan_name}.mp4...")
            file_id = storage.store_mp4_file(
                name=scan_name,
                file_path=self.recorder.final_file,
                metadata={
                    'scanner': 'RepliVision Pi5 Dual Camera',
                    'motor_speed': self.motor.speed,
                    'recording_duration': self.duration,
            ...
            # Queue processing command if enabled
            if QUEUE_PROCESSING_COMMAND:
                print(f"[DRIVE] Queueing processing command...")

                # Construct the command (similar to test_command_queue.py)
                command = (
                    f"run_example "
                    f"--data_path /content/drive/MyDrive/PointClouds3D/{scan_name}.mp4 "
                    f"--save_path /content/drive/MyDrive/PointClouds3D/{scan_name}.ply "
                    f"--interval {PROCESSING_INTERVAL}"
                )

                # Add command to queue
                storage.queue_command(command)
                print(f"[DRIVE] Command queued successfully!")
                print(f"[DRIVE] Command: {command}")

            print(f"[DRIVE] Google Drive upload complete!")
            return True

        except Exception as e:
            print(f"[DRIVE] Error uploading to Google Drive: {str(e)}")
            import traceback
            traceback.print_exc()
            return False
